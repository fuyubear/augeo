...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addChannelOption((option) =>
            option
                ...
                .setDescription(
                    "Pick an empty category channel where the voice accordion will exist."
                )
                .setRequired(false)
        ...
        .addStringOption((option) =>
            option
                .setName("expand_ch_names")
                .setDescription(
                    "Delimiter (default: comma) separated list of " +
        ...
        .addStringOption((option) =>
            option
                .setName("expand_ch_names_delimiter")
                .setDescription(
                    "Delimiter to separate channel names " +
                        "in expand_ch_names."
                )
                .setRequired(false)
        )
        .addBooleanOption((option) =>
            option
                .setName("is_cycle")
                .setDescription(
                    "Are channel names pulled from the front of the expand_ch list?"
                )
                .setRequired(false)
        )
        .addStringOption((option) =>
            option
                .setName("extra_ch_names")
                .setDescription(
        ...
        .addIntegerOption((option) =>
            option
                .setName("available_ch_limit")
                .setDescription(
                    "Number of voice accordion channels " +
    ...
    async execute(interaction) {
        ...
        await keyv
            .get(VOICE_ACCORDION_KEY_URL)
            .then((ret) => (accordionSettings = ret));
        if (!accordionSettings || !accordionSettings.enabled) {
            const voiceAccordionCategory =
                interaction.options.getChannel("category_channel");
            const voiceAccordionBase =
                interaction.options.getString("base_ch_name");
            let dynamicVoiceChannelNames =
                interaction.options.getString("expand_ch_names");
            let delimiter = interaction.options.getString(
                "expand_ch_names_delimiter"
            );
            let voiceAccordionIgnore =
                interaction.options.getString("extra_ch_names");
            const voiceAccordionIsCycle =
                interaction.options.getBoolean("is_cycle");
            const voiceAccordionAvailChLimit =
                interaction.options.getInteger("available_ch_limit");

            if (
                !voiceAccordionBase ||
                !voiceAccordionCategory ||
                !dynamicVoiceChannelNames
            ) {
                await interaction
                    .editReply(
                        "Voice accordion is currently disabled. " +
            ...
            dynamicVoiceChannelNames =
                dynamicVoiceChannelNames.split(delimiter);
            ...
            if (accordionSettings.enabled) {
                ...
                logger.info(
                    `Voice accordion is enabled for guild ${interaction.guildId}`
                );

                await interaction
                    .editReply(
                        "Voice accordion is enabled. " +
            ...
            if (!accordionSettings.enabled) {
                logger.info(
                    `Voice accordion is disabled for guild ${interaction.guildId}`
                );
                await interaction
                    .editReply(
                        "Voice accordion is disabled. All settings " +
        ...
        logger.error(
            `DB error: Failed to toggle voice accordion setting for guild ${interaction.guildId}`
        );
        await interaction
            .editReply("DB error: Failed to toggle voice accordion setting.")
            .catch((err) => logger.error(err));
