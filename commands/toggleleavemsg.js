...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Toggle user leave messages in the System Messages text channel.'),
    async execute(interaction) {
        ...
        if (!enabled || !enabled.flag) {
            ...
            if (!(botPermissionsInSystemMessagesChannel.has(
                ...
                logger.info(`Leave messages failed to toggle for guild ${interaction.guildId} due to lack of permissions`);
            ...
            await keyv.get(LEAVE_MSG_KEY_URL)
                .then(ret => enabled = ret);
            if (enabled.flag) {
                logger.info(`User leave messages functionality is enabled for guild ${interaction.guildId}`);
            ...
            await keyv.get(LEAVE_MSG_KEY_URL)
                .then(ret => enabled = ret);
            if (!enabled.flag) {
                logger.info(`User leave messages functionality is disabled for guild ${interaction.guildId}`);
        ...
        logger.error(`DB error: Failed to toggle the user leave messages setting for guild ${interaction.guildId}`);
        await interaction.editReply('DB error: Failed to toggle the leave messages setting.')
            .catch(err => logger.error(err));
    },
};
