...
module.exports = {
    data: new SlashCommandBuilder()
            ...
            subcommand
                ...
                .addRoleOption((option) =>
                    option
                        .setName("role")
                        .setDescription(
                            "Role to add user to (must be a manager for this role)."
                        )
                        .setRequired(true)
                )
                .addStringOption((option) =>
                    option
                        .setName("reason")
                        .setDescription(
                            "Optionally add a reason for adding this user"
                        )
                        .setRequired(false)
                )
        )
            ...
            subcommand
                ...
                .addRoleOption((option) =>
                    option
                        .setName("role")
                        .setDescription(
                            "Role to remove user from (must be a manager for this role)."
                        )
                        .setRequired(true)
                )
                .addStringOption((option) =>
                    option
                        .setName("reason")
                        .setDescription(
                            "Optionally add a reason for removing this user"
                        )
                        .setRequired(false)
                )
        )
            ...
            subcommand
                ...
                .addRoleOption((option) =>
                    option
                        .setName("role")
                        .setDescription(
                            "Role to add manager to (must have the Manage Roles permission for this role)."
                        )
                        .setRequired(true)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName("rm_manager")
                .setDescription(
                    "Remove a manager from a role you are managing."
                )
                .addUserOption((option) =>
                    option
                        .setName("manager")
                ...
                .addRoleOption((option) =>
                    option
                        .setName("role")
                        .setDescription(
                            "Role to remove manager from (must have the Manage Roles permission for this role)."
                        )
                        .setRequired(true)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .setDescription(
                    "Add a manager role to a role you are managing."
                )
                .addRoleOption((option) =>
                    option
                ...
                .addRoleOption((option) =>
                    option
                        .setName("role")
                        .setDescription(
                            "Role to add manager role to (must have the Manage Roles permission for this role)."
                        )
                        .setRequired(true)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName("rm_manager_role")
                .setDescription(
                    "Remove a manager role from a role you are managing."
                )
                .addRoleOption((option) =>
                    option
                        .setName("manager_role")
                ...
                .addRoleOption((option) =>
                    option
                        .setName("role")
                        .setDescription(
                            "Role to remove manager role from (must have the Manage Roles permission for this role)."
                        )
                        .setRequired(true)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName("info")
                .setDescription(
                    "View details from a role, like managers and members."
                )
                .addRoleOption((option) =>
                    option
                        .setName("role")
        ...
        .addSubcommand((subcommand) =>
            subcommand
                .setName("view_all_managers")
                .setDescription(
                    "List all role managers. Only executable by the Guild/Server Owner and Bot Admins."
                )
                .addStringOption((option) =>
                    option
                        .setName("confirm")
                        .setDescription(
                            "WARNING: All manager user/roles will be pinged (unsanitized output)! Continue?"
                        )
                        .setRequired(false)
                )
        )
            ...
            subcommand
                ...
                .addRoleOption((option) =>
                    option
                        .setName("role")
                        .setDescription(
                            "Role to remove all managers from (must have the Manage Roles permission for this role)."
                        )
                        .setRequired(true)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName("reset_all")
                .setDescription(
                    "Remove all role managers. Only executable by the Guild/Server Owner and Bot Admins."
                )
                .addStringOption((option) =>
                    option
                        .setName("confirm")
    ...
    async executeThis(interaction, plainRoleId) {
        ...
        if (
            ...
            await interaction
                .editReply(
                    "I do not have any permission to Manage Roles in this server."
                )
                .catch((err) => logger.error(err));
            return;
        }
        ...
        switch (interaction.options.getSubcommand()) {
            ...
            case "rm_manager_role":
                if (interaction.options.getRole("role")) {
                    const roleValid = await isValidRoleForManagement(
                        interaction,
                ...
                if (interaction.options.getRole("manager_role")) {
                    const roleValid = await isValidRoleForManagement(
                        interaction,
                        interaction.options.getRole("manager_role"),
                        true
                    );
                    if (!roleValid) {
                        return;
                ...
                if (
                    ...
                    await interaction
                        .editReply(
                            "You do not have any permission to Manage Roles in this server."
                        )
                        .catch((err) => logger.error(err));
                    return;
                }
            ...
            case "rm":
                ...
                if (!managerCheck) {
                    await interaction
                        .editReply("You are not a manager for this role.")
                        .catch((err) => logger.error(err));
                    return;
                }
                break;
            ...
            case "reset_all":
                if (
                    ...
                    await interaction
                        .editReply("Only executable by the Guild/Server Owner.")
                        .catch((err) => logger.error(err));
                    return;
                }
                break;
 ...
 */
async function isValidRoleForManagement(
    interaction,
    roleSelected,
    isManagerRole
) {
    if (roleSelected.name === "@everyone") {
        await interaction
            .editReply("I cannot manage the `@everyone` role.")
            .catch((err) => logger.error(err));
        return false;
    } else if (roleSelected.managed) {
        await interaction
            .editReply(
                "I cannot manage roles managed by integrations like bots."
            )
            .catch((err) => logger.error(err));
        return false;
    }
...
async function isManager(interaction, resolvedRole) {
    ...
    let roleManagers;
    await keyv
        .get(`role/${interaction.guildId}/${roleExamine}/manager`)
