...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName("reason")
                .setDescription(
                    "Add a reason for creating this server invite. May be required."
                )
                .setRequired(false)
        ),
    async execute(interaction) {
        ...
        const lockFileName =
            "lock-invite-" + interaction.guildId + "-" + interaction.member.id;
        ...
        await keyv
            .get(USER_INVITE_USER_STATE_KEY_URL)
        ...
        for (let i = 0; i < userInviteUserState.uses.length; i++) {
            if (
                userInviteUserState.uses[i] +
                    userInviteGuildSettings.useExpiration <=
                timestamp
            ) {
        ...
        await keyv
            .set(USER_INVITE_USER_STATE_KEY_URL, userInviteUserState)
            .catch((err) => logger.error(err));

        // Make sure invites are available for use.
        if (
            userInviteGuildSettings.useLimit > 0 &&
            userInviteUserState.uses.length >= userInviteGuildSettings.useLimit
        ) {
            const secondsUntilNextUse =
                userInviteUserState.uses[0] +
                userInviteGuildSettings.useExpiration -
                timestamp;
            await interaction
                .editReply(
                    "You have used your entire invite creation allotment. You must wait " +
                        secondsUntilNextUse +
                        " seconds before you can create another invite or ask someone else to create one for you."
                )
                .catch((err) => logger.error(err));
        ...
        if (userInviteGuildSettings.requireReason && reason == null) {
            await interaction
                .editReply(
                    "You must provide the reason you're creating the invite."
                )
                .catch((err) => logger.error(err));
            await unlink(lockFileName).catch((err) => logger.error(err));
            logger.info(instanceLogPrefix + " Ending Invite processing.");
        ...
        if (reason) {
            reason =
                `User ${interaction.user.id} created this invite: ` + reason;
        } else {
        ...
        await channel
            .createInvite({
                maxAge: userInviteGuildSettings.expireAfter,
                maxUses: 1,
                unique: true,
                reason: reason,
            })
        ...
        if (inviteSuccess === true) {
            ...
            await interaction
                .editReply(
                    "I've successfully created a single-use " +
            ...
            const msg =
                "I've failed to create an invite. Please make sure I have the permissions to create an invite.";
            logger.error(msg);
