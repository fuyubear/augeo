...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName("resource_lock_name")
                .setDescription("The name of the resource lock to release.")
                .setRequired(true)
        )
        .addStringOption((option) =>
            option
                .setName("resource_lock_arg1")
                .setDescription(
                    "First argument specifically describing the resource lock."
                )
                .setRequired(false)
        ),
    async execute(interaction) {
        ...
        if (!interaction.options.getString("resource_lock_name")) {
            await interaction
                .editReply("You did not provide the resource lock name.")
        ...
        let lockName =
            "lock-" +
            interaction.options.getString("resource_lock_name") +
            "-" +
            interaction.guild.id;

        if (interaction.options.getString("resource_lock_arg1")) {
            lockName +=
                "-" + interaction.options.getString("resource_lock_arg1");
        }
        ...
        if (unlockSuccess) {
            await interaction
                .editReply("I successfully released this resource lock.")
                .catch((err) => logger.error(err));
        } else {
            await interaction
                .editReply("I failed to release this resource lock.")
                .catch((err) => logger.error(err));
        }
    },
};
