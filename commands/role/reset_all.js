...
module.exports.execute = async function (interaction) {
    ...
    if (!interaction.options.getString("confirm")) {
        await interaction
            .editReply(
                "Please provide confirmation to start the resetting process for all roles in this guild."
            )
        ...
        for (const role in roles) {
            await keyv.set(
                `role/${interaction.guildId}/${roles[role].id}/manager`,
                undefined
            );
        }
