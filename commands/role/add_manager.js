...
module.exports.execute = async function (interaction) {
    ...
    if (
        ...
        const msg = `Successfully added manager user ${interaction.options
            .getMember("manager")
            .toString()} to ${interaction.options.getRole("role").toString()}`;
        // PRIVACY: logger.info(msg);
        await interaction.editReply(msg).catch((err) => logger.error(err));
    } else {
        const msg = `DB error: Could not add manager user ${interaction.options
            .getMember("manager")
            .toString()} to ${interaction.options.getRole("role").toString()}`;
